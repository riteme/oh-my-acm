...
void predo () {
	...
	for ( i = 1 ; i <= tot ; i++ ) {
		for ( j = cnt ; prime[i] * prime[i] <= value[j] ; j-- ) {
			...
			if ( h1[j] < 0 ) h1[j] += mod;
			//h[k][j] = h[k][j] - (prime[i]^k) * (h[k][pos...]-sumh[k][i-1]) MODIFY
		}
	}
	for ( i = 1 ; i <= cnt ; i++ )//f(p)=p-1
		h[i] = ( h0[i] - h1[i] + mod ) % mod; //MODIFY
}
LL getf ( LL p , LL e ) {
	return p ^ e;//MODIFY
}
