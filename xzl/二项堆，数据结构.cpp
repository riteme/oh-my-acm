...
struct Heap {
    ...
    Heap() { memset(tr, 0, sizeof(tr)); }
    TNode tr[LOGN];
    auto operator[](int i) -> TNode& { return tr[i]; }
    auto operator[](int i) const -> TNode { return tr[i]; }
};
...
struct HeapInterface {
    ...
    void pop() {
        ...
        q[t] = NULL;
        THeap p;
        for (i = 0, x = x->ch; x; x = x->nxt, i++) p[i] = x;
        reverse(&p[0],&p[i]);
        q = meld(q, p);
        size--;
    }
